...
from yastn.tn.fpeps.ctm import nn_avg, ctmrg, one_site_avg, measure_one_site_spin, Local_CTM_Env, CtmEnv
try:
...
def evolution_hole_Hubbard(lattice, boundary, beta_gs, purification, xx, yy, D_gs, D_evol, sym, chi, interval, beta_end, dbeta, U, mu_up, mu_dn, t_up, t_dn, step, tr_mode, fix_bd):
    ...
    purification='False'
    dims=(xx, yy)
    psi_gs = peps.Peps(lattice, dims, boundary)  # shape = (rows, columns)
    ...
    sv_beta = round(beta_gs * yastn.BETA_MULTIPLIER)  # beta_gs is the temperature at which the ground state has converged
    dict_list_env = []
    ...
    for ms in psi_gs.sites():
        dict_list_env.extend([('cortl', ms), ('cortr', ms), ('corbl', ms), ('corbr', ms), ('strt', ms), ('strb', ms), ('strl', ms), ('strr', ms)])
        
    state1 = np.load("ctm_environment_beta_%1.5f_chi_%1.1f_%s.npy" % (beta_gs, chi, file_name), allow_pickle=True).item()
    env = {ind: yastn.load_from_dict(config=fid.config, d=state1[(*ind, sv_beta)]) for ind in dict_list_env}
    ...
    state = np.load("neel_initialized_Hubbard_spinful_tensors_%s.npy" % (file_name), allow_pickle=True).item()
    for ms in psi_gs.sites():
        psi_gs[ms] = yastn.load_from_dict(config=fid.config, d=state.get((ms, sv_beta))) 
        psi_gs[ms] = psi_gs[ms].unfuse_legs(axes=(0, 1))      
    ...
    target_site = (round((psi_gs.Nx-1)*0.5), round((psi_gs.Ny-1)*0.5))   # middle of an odd x odd lattice

    n_up_bf = exp_1s(psi_gs[target_site], target_site, env1, n_up)
    n_dn_bf = exp_1s(psi_gs[target_site], target_site, env1, n_dn)
    n_int_bf = exp_1s(psi_gs[target_site], target_site, env1, n_int)
    h_bf = exp_1s(psi_gs[target_site], target_site, env1, (fid-n_up)@(fid-n_dn))
    ...
    c = fc_up.add_leg(s=-1) # creates hole
    leg = psi_gs[target_site].get_legs(axes=-1)
    _, leg = yastn.leg_undo_product(leg) # last leg of A should be fused
    ffid = yastn.eye(config=psi_gs[target_site].config, legs=[leg, leg.conj()]).diag() # ffid is the identity read from the ancilla

    cid = yastn.ncon((c, ffid), ((-0, -2, -4), (-1, -3))) # s x iden
    cid = cid.swap_gate(axes=(3, 4))    # swap of connecting axis with ancilla is always in G gate
    print(cid.get_signature())

    cid = cid.fuse_legs(axes=((0, 1), (2, 3), 4)) 

    psi_gs[target_site] = yastn.tensordot(psi_gs[target_site], cid, axes= (4, 1))  # t l b r [s a] str
    psi_gs[target_site] = psi_gs[target_site].fuse_legs(axes=(0, 1, 2, 3, 5, 4)) # t l b r str [s a]

    n_up_af = exp_1s(psi_gs[target_site], target_site, env1, fid*1e-16+n_up) 
    n_dn_af = exp_1s(psi_gs[target_site], target_site, env1, fid*1e-16+n_dn)
    n_int_af = exp_1s(psi_gs[target_site], target_site, env1, (fid*1e-16+n_up)@(fid*1e-16+n_dn))
    h_af = exp_1s(psi_gs[target_site], target_site, env1, (fid-n_up)@(fid-n_dn)) 
    ...
    psi_gs[target_site] = psi_gs[target_site].unfuse_legs(axes=5).fuse_legs(axes=(0, 1, 2, 3, 5, (6, 4))).fuse_legs(axes=(0, 1, 2, 3, (4, 5))) # t l b r [s [a str]]
    for ms in psi_gs.sites():
        psi_gs[ms] = psi_gs[ms].fuse_legs(axes=((0, 1), (2, 3), 4)) 
    ...
    mdata = {}
    file_name = "c_up_shape_%s_Nx_%1.0f_Ny_%1.0f_boundary_%s_purification_%s_fixed_bd_%1.1f_%s_%s_Ds_%s_U_%1.2f_MU_UP_%1.5f_MU_DN_%1.5f_T_UP_%1.2f_T_DN_%1.2f_%s" % (lattice, dims[1], dims[0], boundary, purification, fix_bd, tr_mode, step, D_evol, U, mu_up, mu_dn, t_up, t_dn, sym)
    opts_svd_ntu = {'D_total': D_evol, 'tol_block': 1e-15}
    ...
    for nums in range(time_steps):
        ...
        if (nums + 1) % int(obs_int) == 0:
            ...
            mdata.update(x)
            np.save("hole_initialized_real_time_evolution_Hubbard_model_spinful_tensors_%s.npy" % (file_name), mdata)

        psi, info =  evolution_step_(psi, gates, step, tr_mode, env_type='NTU', opts_svd=opts_svd_ntu, flag='hole')
        print(info)
...
if __name__== '__main__':
    ...
    parser.add_argument("-BETA_GS", type=float, default=0.2) # beta of initial tensor corresponding to ground state
...
# to run, type in terminal : taskset -c 0-13 nohup python -u NTU_spinful_Hubbard_hole_motion_up.py -L 'rectangle' -x 5 -y 5 -BETA_GS 5.0 -B 'finite' -p 'False' -D_gs 6 -X 20 -D_evol 12 -S 'U1xU1xZ2' -I 0.05 -D_BETA 0.005 -BETA_END 10 -U 12 -MU_UP 0.0 -MU_DOWN 0.0 -T_UP 1 -T_DOWN 1 -STEP 'two-step' -MODE 'optimal' -FIXED 0 > hole_motion_spinfull_5_5_beta_gs_5_D_12_U_12_T_1.out &
