...
from yastn.tn.fpeps.ctm import sample, nn_avg, ctmrg, init_ones
...
def benchmark_NTU_hubbard(lattice, boundary, purification, xx, yy, D, sym, mu, t, beta_target, dbeta, chi, num_iter, step, tr_mode, fix_bd):
    ...
    for itr in range(num_iter):
        ...
        print('out: ',out)
        out = {(0, 1): 1, (1, 1): 1, (0, 0): 1, (1, 0): 1}
        psi = initialize_post_sampling_spinless(fc, fcdag, net, out)

        env_ones = init_ones(psi, tc = ((0,) * cfg.sym.NSYM,), Dc=(1,))
        ops = {'cdagc': {'l': fcdag, 'r': fc}}
        ob_hor, ob_ver = nn_avg(psi, env_ones, ops)
       
        xcc
        nn_CTM = 0.25 * (abs(ob_hor.get('cdagc')) + abs(ob_ver.get('cdagc')) + abs(ob_hor.get('ccdag')) + abs(ob_ver.get('ccdag')))

        with open("energy_collapsed_state_spinless_target_beta_%1.2f_%s.txt" % (beta_target,file_name), "a+") as f:
                f.write('{:.0f} {:.5f}\n'.format(itr+1, nn_CTM))
...
# to run, type in terminal : taskset -c 14-20 nohup python -u NTU_spinless_METTS.py -L 'rectangle' -B 'finite' -p 'True' -x 4 -y 4 -D 5 -X 20 -S 'U1' -M 0 -T 1.0 -DBETA 0.005 -BT 4 -ITER 40 -STEP 'two-step' -MODE 'optimal' -FIXED 0 > METTS_spinless_finite_4_4_MU_0_D_5_BETA_4.out &
