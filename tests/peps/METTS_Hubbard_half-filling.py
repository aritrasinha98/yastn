...
from yastn.tn.fpeps import initialize_post_sampling_spinful_sz_basis, initialize_Neel_spinful
...
def NTU_hubbard_HF_METTS(lattice, boundary, purification, xx, yy, D, sym, mu_up, mu_dn, U, t_up, t_dn, beta_target, dbeta, chi, num_iter, step, tr_mode, fix_bd):
    ...
    psi = initialize_Neel_spinful(fc_up, fc_dn, fcdag_up, fcdag_dn, net) # this initialization enforces a filling of n=0.875
...
if __name__== '__main__':
    ...
    parser.add_argument("-D", type=int, default=6)            # bond dimension or distribution of virtual legs of peps tensors,  input can be either an 
    ...
    parser.add_argument("-BT", type=float, default=0.01)        # target inverse temperature beta
    parser.add_argument("-DBETA", type=float, default=0.005)      # dbeta
    parser.add_argument("-X", type=int, default=30)        # chi --- environmental bond dimension for CTM
